...
link insert(link t, unsigned char key)
{
	...
	if (t->item > key)
		t->l = insert(t->l, key); //insert to left subtree
	else
		t->r = insert(t->r, key); //insert to right subtree
...
link delete(link t, unsigned char key)
{
	...
	if (!t)
		return NULL;
	if (t->item > key)
		t->l = delete(t->l, key); //delete from left subtree
	else if (t->item < key)
		t->r = delete(t->r, key); //delete from right subtree
	else {
		//t->item == key
		if (t->l == NULL && t->r == NULL) { //if t is leaf node
			...
			for (p = t->l; p->r; p = p->r);
			t->item = p->item; //replace t with the rightmost node in left subtree
			t->l = delete(t->l, t->item);
		} else { //if t has right subtree
			for (p = t->r; p->l; p = p->l);
			t->item = p->item; //replace t with the leftmost node in right subtree
